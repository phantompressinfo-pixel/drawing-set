...
when the local code only names it, plus IEBC and ADA, which `${CLAUDE_PLUGIN_ROOT}/code-library/`
...
### Two different warnings — they mean opposite things

**1. `*** AMENDED LOCALLY by Aspen/Pitkin ***`** — we hold the model text, and
Aspen or Pitkin has changed it. **The text shown is superseded.** Read the
amendment in `${CLAUDE_PLUGIN_ROOT}/code-library/aspen/title-8-…` or `${CLAUDE_PLUGIN_ROOT}/code-library/pitkin/title-11-…`
and cite that.

`${CLAUDE_PLUGIN_ROOT}/skills/us-building-codes/references/locally-amended-sections.txt` drives this — **306 sections** across
IBC, IEBC, IPC, IMC, IFGC and IRC, regenerated by
`${CLAUDE_PLUGIN_ROOT}/skills/us-building-codes/scripts/extract_amendments.py`.

**2. `*** NOT ICC MODEL TEXT — Colorado <edit|deletion|addition> ***`** — the
stored text is *itself* Colorado-amended, and Aspen (§8.36.010, §8.28.010) and
Pitkin (§11.16.010, §11.28.010) adopt the IPC and IFGC "as published by the
International Code Council", which we do **not** hold. **Use the ICC book for
that section.**

`${CLAUDE_PLUGIN_ROOT}/skills/us-building-codes/references/colorado-dpo-amended.txt` drives this — **340 sections**:

| Code | Colorado's publication | Usable as ICC text? |
| --- | --- | --- |
| IMC | DFPC Mechanical Code 2021 — 0 of 1,716 sections amended | **Yes, clean** |
| IPC | DPO Plumbing Code 2021 — 204 amended | **Only where unflagged** |
| IFGC | DPO Fuel Gas Code 2021 — 136 amended | **Only where unflagged** |

The IPC/IFGC amendments are **not** confined to administrative chapters — IFGC
ch. 4 (gas piping) is 45/251 amended and IPC ch. 13 is 64/126. No jurisdiction
on up.codes publishes an unamended IPC or IFGC; plumbing and fuel gas are
essentially always state-amended.

Both lists are a **floor**. They catch section numbers *named* in the ordinances.
Prose amendments that name no number are invisible, and an ordinance typo carries
through — Pitkin writes "Section 9043.1 Roof Extension", which does not exist
(it means IPC 904.1), so 904.1 goes unflagged. Re-run
`extract_amendments.py` after any re-supplement, and check Title 8 / Title 11
directly for anything submittal-critical.

## What is stored

`${CLAUDE_PLUGIN_ROOT}/code-library/us-building-codes/<jurisdiction>/<code>-<year>/chapter-N-*.csv`

| Jurisdiction | Codes | Chapters | Notes |
| --- | --- | --- | --- |
| `colorado/` | ibc-2021, irc-2021, iebc-2021, **ipc-2021, imc-2021, ifgc-2021** | IBC 1–35 · IRC 1–44 · IEBC 1–16 · IPC · IMC · IFGC | Matches Aspen's adopted 2021 editions |
...
~24,300 provisions across 217 CSVs. Source: scraped from up.codes, enriched with
...
python3 ${CLAUDE_PLUGIN_ROOT}/skills/us-building-codes/scripts/codesearch.py --category means_of_egress --responsibility architect -j colorado
```
...
- **Locally-parsed chapters** (IBC 19–35, IRC 16–44) — `code_category` and
  ...
  `${CLAUDE_PLUGIN_ROOT}/skills/us-building-codes/scripts/backfill_metadata.py` from a static table (ch. 23 is Wood, so every
